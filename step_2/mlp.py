...
def mlp_train(data, labels, hidden_layers, last_layer=20, learning_rate=0.01, epochs=1000, tolerance=1e-4, patience=10):
    # Normalização dos dados de entrada
    data = data.T  # Transposta para formato (features, samples)
    labels = labels.T  # Rótulos em formato (samples, classes)
    ...
    for epoch in range(epochs):
        ...
        mse = np.mean(np.sum((y_pred - labels) ** 2, axis=1))  # MSE para múltiplas classes
        ...
        deltas[-1] = (y_pred - labels) * activation_derivative(y_pred)  # Erro na camada de saída
...
def mlp_predict(data, weights):
    ...
    predictions = np.argmax(final_output, axis=1)  # Retorna a classe com maior pontuação
